    ...
    .then((output) => {
        ...
        content = content.map((e) => {
          ...
          return {
            title: e.title,
            content: [
              [...div.querySelectorAll('td')].map((e) => e.innerHTML).filter((e, index) => index == 0 || index % 3 == 0),
              [...div.querySelectorAll('td')].map((e) => e.innerHTML).filter((e, index) => index == 2 || index % 3 == 2),
              [...div.querySelectorAll('td')].map((e) => e.innerHTML).filter((e, index) => index == 1 || index % 3 == 1),
            ]
          }
        })
            ...
            if (el) {
              return `
                                      <div class="menu-item">
                                        ...
                                        <p class="menu-item__description menu-item__description--pink">${e.content[2][index]} мл.</p>
                                    </div>
                                        `
            } else return '';
          }                    ).join('')}
